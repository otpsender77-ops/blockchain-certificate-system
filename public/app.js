...
class CertificateSystem {
    ...
    setupAdminMenuListeners() {
        console.log('🔧 Setting up admin menu listeners');
        // Admin dropdown toggle
        const adminMenuToggle = document.getElementById('adminMenuToggle');
        const adminDropdown = document.getElementById('adminDropdown');
        
        if (adminMenuToggle && adminDropdown) {
            console.log('✅ Admin menu toggle and dropdown found');
            adminMenuToggle.onclick = (e) => {
                e.preventDefault();
                console.log('🖱️ Admin menu toggle clicked');
        ...
        if (profileLink) {
            ...
            profileLink.onclick = (e) => {
                e.preventDefault();
                console.log('🖱️ Profile link clicked');
                adminDropdown.classList.remove('show');
        ...
        if (changePasswordLink) {
            console.log('✅ Change password link found');
            changePasswordLink.onclick = (e) => {
                e.preventDefault();
                console.log('🖱️ Change password link clicked');
                adminDropdown.classList.remove('show');
                this.showChangePasswordModal();
            };
        } else {
            console.log('❌ Change password link not found');
        }
    ...
    showMetaMaskSection() {
        console.log('🔗 Showing MetaMask section');
        const metamaskSection = document.getElementById('metamaskSection');
        if (metamaskSection) {
            metamaskSection.style.display = 'block';
            console.log('✅ MetaMask section displayed');
        } else {
            console.error('❌ MetaMask section not found');
        }
        this.updateLoginButton();
    }
    ...
    async completeLogin() {
        ...
        console.log('Temp credentials:', this.tempCredentials ? 'Present' : 'Missing');
        ...
        if (!this.tempCredentials) {
            console.log('❌ No temporary credentials');
        ...
        try {
            ...
            if (data.success && data.step === 2 && data.token) {
                ...
                console.log('✅ Complete login successful, showing admin dashboard');
                this.authToken = data.token;
                this.currentUser = data.user;
                localStorage.setItem('authToken', this.authToken);
